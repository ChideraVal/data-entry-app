...
class SchemaForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        schemas = Schema.objects.filter(name__iexact=name)
        
        if self.instance.pk:
            schemas = Schema.objects.filter(name__iexact=name).exclude(pk=self.instance.pk)
...
class EnvironmentForm(forms.ModelForm):
    ...
    #     required=False,  # Set to True later if user selects doc types
    #     choices=DOCUMENT_TYPE_CHOICES,
    #     widget=forms.CheckboxSelectMultiple,
    #     label="Document types",
    # )

    allowed_senders_text = forms.CharField(
        required=False,
    ...
    def clean_name(self):
        ...
        environments = Environment.objects.filter(name__iexact=name)
        
        if self.instance.pk:
            environments = Environment.objects.filter(name__iexact=name).exclude(pk=self.instance.pk)
