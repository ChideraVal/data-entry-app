...
function confirmDelete(event, envJSON) {
    ...
    // show a more helpful message when the environment has extractions
    const usedBadge = pendingForm.closest('.card')?.querySelector('.badge.used');
    const modalMessage = document.getElementById('modalMessage');
    if (usedBadge) {
        modalMessage.textContent = 'This environment has already processed documents. Deleting it will remove its configuration and any extracted metadata. Are you sure?';
    } else {
        modalMessage.textContent = 'Are you sure you want to delete this environment? This action cannot be undone.';
    }
