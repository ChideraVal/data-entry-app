...
function createRowNode(email) {
    ...
    subj.innerHTML = `<div class="subject">${escapeHtml(email.subject)}</div>
                        <div class="meta">${escapeHtml(email.from)} • ${escapeHtml(email.date)} • ${email.attachments.length} attachments • ${format_bytes(email.total_file_size) || '- KB'}</div>`;
    ...
    if (email.status === 'successful') {
        ...
        tag2.className = 'tag ' + (email.is_approved ? 'success' : 'failed');
        tag2.textContent = email.is_approved ? 'Approved' : 'Not approved';
